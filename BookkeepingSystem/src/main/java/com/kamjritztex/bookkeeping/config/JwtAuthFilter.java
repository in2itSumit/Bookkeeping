...
import com.kamjritztex.bookkeeping.exception.JwtTokenException;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        log.debug("Token recived by request ====="+authHeader);
        ...
        try {
            ...
            log.info("userName extract from jwt========= "+username);

            if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                UserDetails userDetails = this.userDetailsService.loadUserByUsername(username);
                log.info("Role of a user========= "+userDetails.getAuthorities());
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return path.startsWith("/bookeeping/auth/authenticate/login"); //|| path.startsWith("/bookkeeping/auth"); // Adjust path matching as needed
    }
}